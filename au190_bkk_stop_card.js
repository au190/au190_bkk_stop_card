class BKKStopCard extends HTMLElement {
  ...
  _getAttributes(hass, filter1) {
    ...
    filters1[supportedItems*7] = {key: "sensor." + filter1 + ".updated"};
    filters1[supportedItems*7+1] = {key: "sensor." + filter1 + ".items"};
    ...
    attr.forEach(key => {
      ...
      if ( re.test(newkey) ) {
        ...
        updated = attributes.get(key).value;
  ...
  _updateStation(element, attributes) {
    element.innerHTML = `
      ${attributes.map((attribute) => `
        <h3>${attribute.station} <span class="upd">[${attribute.updated}]</span></h3>
      `)[0]}
    `;
  }
